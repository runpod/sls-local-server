...
func sendLogsToTinyBird(logBuffer chan string, done chan struct{}) {
	...
	flush := func() {
		...
		if resp.StatusCode != http.StatusOK {
			...
			log.Error("Tinybird rejected logs", zap.Int("status", resp.StatusCode), zap.String("response", string(body)))
		} else {
...
func RunCommand(command string, ide bool) error {
	...
	go sendLogsToTinyBird(logBuffer, done)
	...
	go func() {
		...
		for {
			...
			if n > 0 {
				logBuffer <- fmt.Sprintf("#ERROR: %s", string(buf[:n]))
			}
			if err == io.EOF {
				break
			}
	...
	if err := cmd.Wait(); err != nil {
		...
		sendResultsToGraphQL("FAILED", &errorMsg)
		log.Error("Command closed", zap.Error(err))
	}
