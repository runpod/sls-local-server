...
.PHONY: build
build:
	go build -ldflags="-X 'sls-local-server/main.Version=$(VERSION)'" -o main main.go

.PHONY: build-arm64
build-arm64:
	env GOARCH=arm64 go build -ldflags="-X 'sls-local-server/main.Version=$(VERSION)'" -o main main.go
